    ...
    fetch('/api/leaderboard')
      ...
      .then((data: any[]) => {
        setRows(data.map((r, i) => ({
          rank: i + 1,
          username: r.username,
          lumens: r.username === 'demo_user' ? balance : Number(r.lumens),
          contributions: Number(r.contributions),
          scaffolds: Number(r.scaffolds),
          rep: Number(r.rep),
          isMe: r.username === 'demo_user',
        })))
      })
      .catch(() => {})
        ...
        {top3.length >= 3 && (
          <div className="grid md:grid-cols-3 gap-4 mb-12">
            ...
            <Card className="p-6 bg-card border-border flex flex-col items-center text-center md:mt-8 order-first md:order-1">
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted text-foreground font-bold text-sm mb-3">{top3[1].username[0].toUpperCase()}</div>
              <h3 className="font-bold text-foreground mb-1">{top3[1].username}</h3>
            ...
            <Card className="p-6 bg-card border-2 border-primary ring-2 ring-primary ring-offset-2 ring-offset-background flex flex-col items-center text-center order-2 md:-mt-4">
              ...
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground font-bold text-lg mb-3">{top3[0].username[0].toUpperCase()}</div>
              <h3 className="font-bold text-foreground mb-1 text-lg">{top3[0].username}</h3>
            ...
            <Card className="p-6 bg-card border-border flex flex-col items-center text-center md:mt-4 order-3">
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted text-foreground font-bold text-sm mb-3">{top3[2].username[0].toUpperCase()}</div>
              <h3 className="font-bold text-foreground mb-1">{top3[2].username}</h3>
        ...
        {me && (
          <Card className="p-6 bg-card border-border border-l-4 border-l-primary">
            ...
            <p className="text-2xl font-bold text-foreground mb-1">
              #{me.rank} <span className="text-sm text-muted-foreground font-normal">demo_user</span>
            </p>
