...
import { X, BookOpen, Users, BarChart2, Layers, Lightbulb, ChevronRight, Clock, Star, ExternalLink } from 'lucide-react'
...
// Estimated read times based on difficulty
...
// Generate bullet "what you'll learn" points from description
function generateLearningPoints(description: string, title: string): string[] {
  // Derive 2-3 bullet points from description text
...
export function NodeDetailDrawer({ node, weaveId, allNodes = [], open, onClose, onUnlock }: NodeDetailDrawerProps) {
  if (!node) return null

  // Find prerequisite nodes (same or lower depth)
  const prerequisites = allNodes.filter(
    (n) => n.id !== node.id && n.depth < node.depth && !n.is_scaffold
  ).slice(-2)

  // Find next nodes (deeper depth)
  const nextNodes = allNodes.filter(
    (n) => n.id !== node.id && n.depth > node.depth
  ).slice(0, 2)
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 z-50 h-full w-full max-w-[420px] bg-card border-l border-border shadow-2xl transition-transform duration-300 ease-out flex flex-col ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed text-muted-foreground bg-background rounded-xl p-4 border border-border">
                {node.description}
              </p>
            </div>
            ...
            {!node.is_scaffold && node.contributed_by && (
              <div className="bg-background rounded-xl p-4 border border-border">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Contributed by</span>
                </div>
        ...
        <div className="p-5 border-t border-border bg-background/50 shrink-0 space-y-2">
          ...
          {node.is_scaffold && onUnlock && (
            <Button
              onClick={() => { onUnlock(node); onClose() }}
              className="w-full h-11 font-bold text-sm"
              style={{ background: 'linear-gradient(135deg, #F59E0B, #F97316)', color: '#000' }}
            >
              ✍ Contribute to this Node · +50 LM
            </Button>
          )}
        </div>
      </div>
